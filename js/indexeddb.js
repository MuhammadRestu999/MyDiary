...
function searchString(keyword,cb){
   ...
   request.onsuccess = function(event) {
       var cursor = event.target.result;
       if (cursor) {
           if (cursor.value.title.indexOf(keyword) !== -1 ||cursor.value.content.includes(keyword)) {                
...
function searchAddress(folder, keyword, cb) {
   ...
   objectStore.index("folder, alphabetical_order").openCursor(range).onsuccess = function (event) {
      var cursor = event.target.result;
      if (cursor) {
           if (cursor.value.name.toLowerCase().includes(keyword) || cursor.value.name.includes(keyword) || cursor.value.number.includes(keyword) || cursor.value.number.replace(/[\+\- \(\)]/g, "").includes(keyword) || cursor.value.number.includes(keyword.replace(/[\+\- \(\)]/g, "")) || cursor.value.number.replace(/[\+\- \(\)]/g, "").includes(keyword.replace(/[\+\- \(\)]/g, ""))) {
               result.push(cursor.value);
           }
           cursor.continue();
       }else{
